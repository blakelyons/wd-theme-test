...
function moduleDetails(module) {
    moduleLookups = {
        ...
        */
        ...
        'component_sponsors' : { layout: 'basic', message: 'Sponsor Items', newitem: function(itemnumber,subitemnumber) { return { title: ('Item ' + itemnumber), appsubitems:[{level:('SubItem ' + subitemnumber)}] }; } },
            ...
            app = new Vue({
                ...
                methods: {
                    additem: function(event) {
                        ...
                        app.appitems.push(moduleDetails(itemtemplate).newitem((app.appitems.length + 1),1));
                    },
                    addsubitem: function(event,index) {
                        if (event) event.preventDefault();
                        app.appitems[index].appsubitems.push(moduleDetails(itemtemplate).newitem((app.appitems[index], app.appitems[index].appsubitems.length + 1)));
                    },
                    deleteitem: function(event,index) {
                        if (event) event.preventDefault();
                        this.$delete(app.appitems, index);
                    },
                    deletesubitem: function(event,index,subindex) {
                        if (event) event.preventDefault();
                        this.$delete(app.appitems[index].appsubitems, subindex);
                    },
                    ...
                    movesubitemup: function(event,index,subindex) {
                        if (event) event.preventDefault();
                        if (subindex > 0) {
                            var itemToMove = app.appitems[index].appsubitems.splice(subindex, 1);
                            app.appitems[index].appsubitems.splice(( subindex - 1), 0, itemToMove[0]);
                        }
                    },
                    ...
                    movesubitemdown: function(event,index,subindex) {
                        if (event) event.preventDefault();
                        if ((index + 1) < app.appitems.length) {
                            var itemToMove = app.appitems[index].appsubitems.splice(subindex, 1);
                            app.appitems[index].appsubitems.splice(( subindex + 1), 0, itemToMove[0]);
                    ...
                    openImagefinderSub: function(event,index,subindex,field,root) {
                        if (event) event.preventDefault();
                        if(root === true) {
                            console.log(root);
                        }
                        console.log('field check');
                        console.log(app.uivars);
                        $('#attr__image_finder').val('');
                        Freedom.UI.elfinderPopup('attr__image_finder', '');
                        app.$set( app.uivars, 'replaceImage', subindex );
                        app.$set( app.appindex, index);
                        if ( !!field ) {
                            app.$set( app.uivars, 'replaceImageField', field );
                        }
                    },
                    ...
                    checkImageFinderSub: function() {
                        setInterval( function(){
                            if ( $('#attr__image_finder').val().length > 1 && app.uivars.replaceImage !== false ) {

                                app.$set( app.appitems[app.appindex.value].appsubitems[app.uivars.replaceImage],  'image', 'clientuploads/directory/' + itemtemplate + '/' + $('#attr__image_finder').val() );
                                app.$set( app.uivars, 'replaceImage', false );
                                app.$set( app.uivars, 'replaceImageField', false );
                                $('#attr__image_finder').val('');
                            }
                    ...
                    openLinkfinderSub: function(event,index,subindex) {
                        if (event) event.preventDefault();
                        $('#attr__link_finder').val('');
                        set_url_init('attr__link_finder');
                        app.$set( app.uivars, 'replaceLink', subindex );
                        app.$set( app.appindex, index);
                    },
                    ...
                    checkLinkFinderSub: function() {
                        setInterval( function(){
                            if ( $('#attr__link_finder').val().length > 1 && app.uivars.replaceLink !== false ) {
                                app.$set( app.appitems[app.appindex.value].appsubitems[app.uivars.replaceLink], 'link', $('#attr__link_finder').val() );
                                app.$set( app.uivars, 'replaceLink', false );
                                $('#attr__link_finder').val('');
                            }
                        }, 250);
                ...
                mounted: function () {
                    this.$nextTick(function () {
                        this.saveData();
                        if (itemtemplate == 'component_sponsors') {
                            this.checkLinkFinderSub();
                            this.checkImageFinderSub();
                        } else {
